...
func dbConn() (db *sql.DB) {
	dbDriver := "mysql"
	dbName := "pert7_50420460" // Diganti menjadi nama database kalian masing-masing
	dbUser := "root"
	dbPass := ""
...
func main() {
	...
	srv := &http.Server{
		Handler:      r,
		Addr:         "127.0.0.1:8060", // 2 angka belakang port diganti menjadi 2 angka dibelakang npm masing-masing
		WriteTimeout: 15 * time.Second,
		ReadTimeout:  15 * time.Second,
	}

	log.Print("Server berjalan di http://127.0.0.1:8060") // 2 angka belakang port diganti menjadi 2 angka dibelakang npm masing-masing
	fmt.Printf("Farhan Hafizh")                           // Diganti dengan nama kalian masing-masing
	srv.ListenAndServe()
}
